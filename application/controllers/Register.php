...
class Register extends CI_Controller
{
    ...
    {
        $registrationsEnabled = FALSE;     //deactivate account functions
        if($registrationsEnabled){        //if account functions are enabled
    ...
    public function home(){
        ...
        $this->form_validation->set_rules("email", "Email", "required|trim|strtolower|max_length[200]|valid_email|is_unique[waitinglist.email]|xss_clean");
        ...
        if ($this->form_validation->run() === FALSE) {
            ...
            $this->load->model('model_participant');
            if ($query = $this->model_participant->create_waitinglist_participant()) {
                ...
                $this->email->bcc('it@esngreece.gr'); //bcc local_coordinator
                $this->email->subject('Welcome to the Crete Trip 2017!');
                $this->email->message('
...
<p>Due to a large number of registrations, you are currently on a waiting list and will be informed if and when you need to pay the event fee.</p>
